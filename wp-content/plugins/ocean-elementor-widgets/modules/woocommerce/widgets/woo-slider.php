...
class Woo_Slider extends Widget_Base {
	...
	public function render() {
		...
		// Get the number of queried products
		$post_count = $query->post_count;
		$slides_to_show = (int) $settings['slides_to_show']['size'];

		// Disable loop if post count is less than or equal to slides to show
		if ( $post_count <= $slides_to_show ) {
			$settings['infinite_loop'] = 'no';
		}
		...
		// Add the post count to the carousel settings
		$carousel_settings['post_count'] = $post_count;

		$this->add_render_attribute( 'oew-woo-slider', 'data-settings', wp_json_encode( $carousel_settings ) );
		$this->add_render_attribute( 'oew-woo-slider', 'id', $slider_id );
		?>
				...
				<ul class="products oceanwp-row swiper-wrapper clr">
					...
					while ( $query->have_posts() ) :
						...
						<li <?php wc_product_class( 'swiper-slide', $product ); ?>>
							<?php
							// Check if we are in Elementor editor mode and if there is only one product
							if ( $post_count <= 1 && \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
								// Skip the first two actions to prevent duplicated image
							} else {
								// Include the first two actions as usual
		...
		<?php
		// Check if not in Elementor editor mode before adding the script.
		if ( \Elementor\Plugin::instance()->editor->is_edit_mode() === false ) :
		?>
		<script>
			(function($) {
				function initializeAllSwipers() {
					// Only proceed if on the cart page
					if (!$('body').hasClass('woocommerce-cart')) {
						return;
					}

					if (typeof elementorFrontend === 'undefined' || typeof elementorFrontend.config === 'undefined') {
						return;
					}

					var elementorBreakpoints = elementorFrontend.config.breakpoints;

					if (!elementorBreakpoints) {
						return;
					}

					var breakpointMapping = {
						'xs': 'mobile_extra',
						'sm': 'mobile',
						'md': 'tablet_extra',
						'lg': 'tablet',
					...
					var sortedBreakpoints = Object.keys(elementorBreakpoints).map(function(key) {
						return {
							label: key,
							value: parseInt(elementorBreakpoints[key])
						};
					}).sort(function(a, b) {
					...
					$('.oew-woo-slider').each(function(index) {
						var $sliderElement = $(this);
						var sliderElement = this;

						// Check if there are slides
						var slides = $sliderElement.find('.swiper-slide');
						var postCount = $sliderElement.data('settings').post_count || 0;
						var slidesToShow = parseInt($sliderElement.data('settings').items) || 1;

						if (slides.length === 0 || postCount <= slidesToShow) {
							// No slides or insufficient slides, ensure Swiper is not initialized
							if (sliderElement.swiper) {
								sliderElement.swiper.destroy(true, true);
							}
							$sliderElement.closest('.oew-woo-carousel').hide();
							return;
						} else {
							$sliderElement.closest('.oew-woo-carousel').show();
						}
						...
						var sliderSettings = $sliderElement.data('settings');
						if (!sliderSettings) {
							return;
						}

						if (typeof sliderSettings === 'string') {
							try {
								sliderSettings = JSON.parse(sliderSettings);
							} catch (e) {
								console.error('Invalid JSON in data-settings:', e);
								return;
							}
						}

						// Generate unique IDs for navigation elements if they don't exist
						var sliderId = $sliderElement.attr('id') || 'swiper-' + index;
						$sliderElement.attr('id', sliderId);

						// Ensure navigation elements exist
						var $container = $sliderElement.closest('.oew-woo-carousel');
						if (sliderSettings.arrows === "1") {
							if (!$container.find('.swiper-button-next-' + sliderId).length) {
								$container.append('<div class="swiper-button-next oew-swiper-buttons swiper-button-next-' + sliderId + '"><svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512.002 512.002" xml:space="preserve"><path d="M388.425,241.951L151.609,5.79c-7.759-7.733-20.321-7.72-28.067,0.04c-7.74,7.759-7.72,20.328,0.04,28.067l222.72,222.105L123.574,478.106c-7.759,7.74-7.779,20.301-0.04,28.061c3.883,3.89,8.97,5.835,14.057,5.835c5.074,0,10.141-1.932,14.017-5.795l236.817-236.155c3.737-3.718,5.834-8.778,5.834-14.05S392.156,245.676,388.425,241.951z"/></svg></div>');
							}
							if (!$container.find('.swiper-button-prev-' + sliderId).length) {
								$container.append('<div class="swiper-button-prev oew-swiper-buttons swiper-button-prev-' + sliderId + '"><svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 443.52 443.52" xml:space="preserve"><path d="M143.492,221.863L336.226,29.129c6.663-6.664,6.663-17.468,0-24.132c-6.665-6.662-17.468-6.662-24.132,0l-204.8,204.8c-6.662,6.664-6.662,17.468,0,24.132l204.8,204.8c6.78,6.548,17.584,6.36,24.132-0.42c6.387-6.614,6.387-17.099,0-23.712L143.492,221.863z"/></svg></div>');
							}
						}

						// Prepare Swiper options, including responsive settings
						var swiperOptions = {
							loop: sliderSettings.loop === "1" ? true : false,
							watchOverflow: true,
							slidesPerView: parseInt(sliderSettings.items) || 1,
							slidesPerGroup: parseInt(sliderSettings.slides) || 1,
							spaceBetween: parseInt(sliderSettings.margin) || 0,
							effect: sliderSettings.effect || 'slide',
							speed: parseInt(sliderSettings.speed) || 500,
							autoplay: (sliderSettings.autoplay && sliderSettings.autoplay !== "0") ? {
								delay: parseInt(sliderSettings.autoplay),
								disableOnInteraction: false
							} : false,
							navigation: sliderSettings.arrows === "1" ? {
								nextEl: '.swiper-button-next-' + sliderId,
								prevEl: '.swiper-button-prev-' + sliderId,
							} : false,
							pagination: sliderSettings.dots === "1" ? {
						...
						// Set loop to false when there's only one slide
						if (slides.length <= 1) {
							swiperOptions.loop = false;
						}

						// Dynamically set breakpoints based on Elementor's settings
						sortedBreakpoints.forEach(function(bp) {
							var label = bp.label;
							var width = bp.value;

							var mappedLabel = breakpointMapping[label];

							if (!mappedLabel) {
								return;
							}

							var slidesPerView = parseInt(sliderSettings['items-' + mappedLabel]) || parseInt(sliderSettings.items) || 1;
							var slidesPerGroup = parseInt(sliderSettings['slides-' + mappedLabel]) || parseInt(sliderSettings.slides) || 1;
							var spaceBetween = parseInt(sliderSettings['margin-' + mappedLabel]) || parseInt(sliderSettings.margin) || 0;

							swiperOptions.breakpoints[width] = {
						...
						// Initialize Swiper with events
						var swiperInstance = new Swiper(sliderElement, {
							...swiperOptions,
							on: {
								init: function() {
									this.update();
						...
						$sliderElement.data('swiper-instance', swiperInstance);
					});
				}

				// Initialize Swipers on document ready
				$(document).ready(function() {
					initializeAllSwipers();
				});

				// Re-initialize Swipers after cart content is updated via AJAX
